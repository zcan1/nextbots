...
-- Sound hooks: add your own files under sound/npc_diddy_blud/ with the names
-- shown below. Files must be 44100 Hz to play correctly.
ENT.JumpSound = {}
ENT.JumpHighSound = {}
ENT.TauntSounds = {}
local chaseMusic = Sound("npc_diddy_blud/diddy_chase_theme.mp3")
local killMusic = Sound("npc_diddy_blud/diddy_kill_theme.mp3")

local workshopID = nil

local IsValid = IsValid

local killFrameFiles = file.Find("materials/npc_diddy_blud/diddy_blud_kill/*.png", "GAME")
table.sort(killFrameFiles)
local KILL_FRAME_COUNT = #killFrameFiles
local KILL_ANIMATION_FPS = 12
local KILL_ANIMATION_DURATION = (KILL_FRAME_COUNT > 0)
        and math.max(KILL_FRAME_COUNT / KILL_ANIMATION_FPS, 1)
        or 0

function ENT:SetupDataTables()
        self:NetworkVar("Bool", 0, "Raging")
        self:NetworkVar("Bool", 1, "Moving")
        self:NetworkVar("Bool", 2, "KillAnimating")
        self:NetworkVar("Float", 0, "KillAnimationStart")
        self:NetworkVar("Float", 1, "KillAnimationEnd")

        if SERVER then
                self:SetRaging(false)
                self:SetMoving(false)
                self:SetKillAnimating(false)
                self:SetKillAnimationStart(0)
                self:SetKillAnimationEnd(0)
        end
end

if SERVER then -- SERVER --

local npc_diddy_blud_acquire_distance =
	CreateConVar("npc_diddy_blud_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which diddy_blud will chase a target.")

local npc_diddy_blud_spawn_protect =
	CreateConVar("npc_diddy_blud_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, diddy_blud will not target players or hide within 200 units of \z
	a spawn point.")

local npc_diddy_blud_attack_distance =
	CreateConVar("npc_diddy_blud_attack_distance", 80, FCVAR_NONE,
	"The reach of diddy_blud's attack.")

local npc_diddy_blud_attack_interval =
	CreateConVar("npc_diddy_blud_attack_interval", 0.2, FCVAR_NONE,
	"The delay between diddy_blud's attacks.")

local npc_diddy_blud_attack_force =
	CreateConVar("npc_diddy_blud_attack_force", 800, FCVAR_NONE,
	"The physical force of diddy_blud's attack. Higher values throw things \z
	farther.")

local npc_diddy_blud_smash_props =
	CreateConVar("npc_diddy_blud_smash_props", 1, FCVAR_NONE,
	"If set to 1, diddy_blud will punch through any props placed in their way.")

local npc_diddy_blud_allow_jump =
	CreateConVar("npc_diddy_blud_allow_jump", 1, FCVAR_NONE,
	"If set to 1, diddy_blud will be able to jump.")

local npc_diddy_blud_hiding_scan_interval =
	CreateConVar("npc_diddy_blud_hiding_scan_interval", 3, FCVAR_NONE,
	"diddy_blud will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant diddy_bluds are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_diddy_blud_hiding_repath_interval =
	CreateConVar("npc_diddy_blud_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to diddy_blud's hiding spot will be redetermined every X seconds.")

local npc_diddy_blud_chase_repath_interval =
	CreateConVar("npc_diddy_blud_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of diddy_blud's target will be redetermined every \z
	X seconds.")

local npc_diddy_blud_expensive_scan_interval =
	CreateConVar("npc_diddy_blud_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_diddy_blud_force_download =
	CreateConVar("npc_diddy_blud_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download diddy_blud resources \z
...
if npc_diddy_blud_force_download:GetBool() then
        if workshopID then
                resource.AddWorkshop(workshopID)
        end

        resource.AddFile("materials/npc_diddy_blud/diddy_blud.vmt")
        resource.AddFile("materials/npc_diddy_blud/killicon.vmt")

        local downloadableAssets = {
                "materials/npc_diddy_blud/diddy_blud.png",
                "materials/entities/npc_diddy_blud.png",
                "sound/npc_diddy_blud/diddy_chase_theme.mp3",
                "sound/npc_diddy_blud/diddy_kill_theme.mp3",
        }

        for _, assetPath in ipairs(downloadableAssets) do
                if file.Exists(assetPath, "GAME") then
                        resource.AddFile(assetPath)
                end
        end

        for _, frame in ipairs(killFrameFiles) do
                resource.AddFile("materials/npc_diddy_blud/diddy_blud_kill/" .. frame)
        end
end

util.AddNetworkString("diddy_blud_nag")
util.AddNetworkString("diddy_blud_navgen")

local function killAnimationTimerName(ent)
        return "diddy_blud_kill_anim_" .. ent:EntIndex()
end

local function emitRandomSound(ent, soundTable, level, pitch)
        if not soundTable or #soundTable == 0 then return end

        local soundName = table.Random(soundTable)
        if soundName then
                ent:EmitSound(soundName, level or 350, pitch or 100)
        end
end
...
local VECTOR_diddy_blud_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_diddy_blud_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_diddy_blud: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other diddy_bluds, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_diddy_blud"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "diddy_bludMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_diddy_blud" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why diddy_blud isn't working.
	net.Start("diddy_blud_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_diddy_blud: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_diddy_blud: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_diddy_blud: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_diddy_blud: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_diddy_blud: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, diddy_blud in pairs(ents.FindByClass("npc_diddy_blud")) do
		diddy_blud:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "diddy_bludNavGen", navEndGenerate)
	else
		print("npc_diddy_blud: nav_generate failed to initialize")
...
concommand.Add("npc_diddy_blud_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_diddy_blud: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. diddy_blud doesn't use
	...
	elseif isConsole then
		print("npc_diddy_blud: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:TriggerKillAnimation()
        if KILL_ANIMATION_DURATION <= 0 then return end

        local startTime = CurTime()
        self:SetKillAnimating(true)
        self:SetKillAnimationStart(startTime)
        self:SetKillAnimationEnd(startTime + KILL_ANIMATION_DURATION)

        timer.Create(killAnimationTimerName(self), KILL_ANIMATION_DURATION, 1, function()
...
function ENT:OnRemove()
        ...
        timer.Remove(killAnimationTimerName(self))
end

function ENT:GetNearestTarget()
	-- Only target entities within the acquire distance.
	local maxAcquireDist = npc_diddy_blud_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_diddy_blud_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_diddy_blud_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_diddy_blud_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. diddy_blud means business.
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_diddy_blud_attack_distance:GetInt()
	...
	then
		...
		self.loco:SetJumpHeight(300)
                ...
                local jumpSounds = (jumpHeight > HIGH_JUMP_HEIGHT) and self.JumpHighSound or self.JumpSound
                emitRandomSound(self, jumpSounds, 350, 100)
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_diddy_blud_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_diddy_blud_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_diddy_blud_attack_distance:GetInt()
			if self:AttackNearbyTargets(attackDistance) then
				if currentTime - self.LastTaunt > TAUNT_INTERVAL then
					self.LastTaunt = currentTime
                                    emitRandomSound(self, self.TauntSounds, 350, 100)
				end
		...
		local repathInterval = npc_diddy_blud_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_diddy_blud_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_diddy_blud_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_diddy_blud_hiding_repath_interval:GetFloat()
...
local MAT_diddy_blud = Material("npc_diddy_blud/diddy_blud", "smooth unlitgeneric")
local killMaterials = {}
for _, frame in ipairs(killFrameFiles) do
        local materialPath = "npc_diddy_blud/diddy_blud_kill/" .. frame
        killMaterials[#killMaterials + 1] = Material(materialPath, "smooth unlitgeneric")
end

killicon.Add("npc_diddy_blud", "npc_diddy_blud/killicon", color_white)
language.Add("npc_diddy_blud", "Diddy Blud")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_diddy_blud: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a diddy_blud.
local killMusicHandle = nil

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_diddy_blud_music_volume =
        CreateConVar("npc_diddy_blud_music_volume", 1,
        bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
        "Maximum music volume when being chased by diddy_blud. (0-1, where 0 is muted)")

-- If another diddy_blud comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, diddy_bluds do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_diddy_blud_PANIC_COUNT diddy_bluds are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of diddy_blud.
local MUSIC_diddy_blud_PANIC_COUNT = 8

local MUSIC_diddy_blud_MAX_DISTANCE_SCORE =
        (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_diddy_blud_PANIC_COUNT

local function ensureKillMusicHandle()
        if killMusicHandle == nil then
                if not IsValid(LocalPlayer()) then return nil end

                killMusicHandle = CreateSound(LocalPlayer(), killMusic)
                killMusicHandle:Stop()
        end

        return killMusicHandle
end

local function updatePanicMusic()
        if not chaseMusic then return end
        if #ents.FindByClass("npc_diddy_blud") == 0 then
                -- Whoops. No need to run for now.
                DevPrint(4, "Halting music timer.")
                timer.Remove("diddy_bludPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_diddy_blud_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_diddy_blud" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_diddy_blud_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear diddy_blud taunt us.
...
local function startTimer()
        if not chaseMusic then return end
        if not timer.Exists("diddy_bludPanicMusicUpdate") then
                timer.Create("diddy_bludPanicMusicUpdate", 0.05, REPEAT_FOREVER,
                        updatePanicMusic)
...
function ENT:UpdateKillMusicState()
        local handle = ensureKillMusicHandle()
        if not handle then return end

        local userVolume = math.Clamp(npc_diddy_blud_music_volume:GetFloat(), 0, 1)
        if self:GetKillAnimating() and userVolume > 0 then
                if not self._killMusicPlaying then
                        handle:Stop()
                        handle:Play()
                        handle:ChangeVolume(userVolume, 0)
                        self._killMusicPlaying = true
                end
        elseif self._killMusicPlaying then
                handle:Stop()
                self._killMusicPlaying = false
        end
end

function ENT:Think()
        self:UpdateKillMusicState()
        self:SetNextClientThink(CurTime())
        return true
end

local DRAW_OFFSET = SPRITE_SIZE / 2 * vector_up
local function SelectDrawMaterial(ent)
        if ent:GetKillAnimating() then
                local elapsed = math.max(0, CurTime() - ent:GetKillAnimationStart())

                if #killMaterials > 0 then
                        local frameIndex = math.Clamp(math.floor(elapsed * KILL_ANIMATION_FPS) + 1,
                                1, #killMaterials)
                        return killMaterials[frameIndex]
                end
        end

        return MAT_diddy_blud
end

function ENT:DrawTranslucent()
        render.SetMaterial(SelectDrawMaterial(self))

        -- Get the normal vector from diddy_blud to the player's eyes, and then compute
	...
	-- diddy_blud should only look 1/3 of the way up to the player so that they
...
function ENT:OnRemove()
        if self._killMusicPlaying then
                local handle = ensureKillMusicHandle()
                if handle then
                        handle:Stop()
                end
...
surface.CreateFont("diddy_bludHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("diddy_bludHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("diddy_blud is studying this map.", "diddy_bludHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "diddy_bludHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "diddy_bludHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "diddy_bludHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "diddy_bludHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("diddy_blud_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "diddy_bludNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. diddy_blud doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_diddy_blud_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for diddy_blud to figure \z
		this map out.\n\z
...
net.Receive("diddy_blud_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! diddy_blud doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet diddy_blud-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! diddy_blud doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to diddy_blud.\n\z
			\n\z
			If you ARE the server host, you can run npc_diddy_blud_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not diddy_blud-compatible!",
			"Ok", nil,
...
--
list.Set("NPC", "npc_diddy_blud", {
	Name = "diddy_blud",
	Class = "npc_diddy_blud",
	Category = "Nextbot",
	AdminOnly = true
})
